...
def plots(train_costs, val_costs, test_costs, n, epochs=1000):
    #Plots
    plt.figure()
    ...
    plt.title('Train and Val Errors at n = %d'%n)
    ymin = val_costs.min()
    ...
    plt.legend()
    plt.savefig('./newest_n/' + str(n) + '_train_val_mse.png')
    plt.show()
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Mean Squared Error')
    plt.title('Test Errors at n = %d'%n)
    ymin = test_costs.min()
    ...
    kw = dict(xycoords='data',textcoords="axes fraction",
              arrowprops=arrowprops, bbox=bbox_props, ha="right", va="center")
    plt.gca().annotate(text, xy=(xmin,ymin), xytext=(0.94,0.6), **kw)
    plt.savefig('./newest_n/' + str(n) + '_test_error.png')
    plt.show()
...
for no_hidden in no_hidden_list:
    test_costs = []
    train_costs = []
    val_costs = []
    ...
    plots(np.mean(train_costs, axis=0), np.mean(val_costs, axis=0), np.mean(test_costs, axis=0), no_hidden)
